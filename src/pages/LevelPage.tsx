...
import { ArrowLeft, Send, CheckCircle, XCircle, AlertTriangle, MessageSquare, Brain, Lightbulb } from 'lucide-react';
...
export const LevelPage: React.FC = () => {
  ...
  const { loadLevel, loadTasks, loadOptions, currentLevel, tasks, options, submitAnswer, startLevel, completeLevel } = useGame();
  ...
  const evaluateReasoningWithAI = async (
    ...
    try {
      ...
      return generateBasicFallbackFeedback(studentReasoning, isCorrect);
    } finally {
  ...
  const generateLowQualityFeedback = (reasoning: string, isCorrect: boolean, termScore: number, hasExplanation: boolean) => {
  ...
  const generateMediumQualityFeedback = (reasoning: string, isCorrect: boolean, termScore: number, hasExplanation: boolean, usedCategories: number) => {
    let feedback = '';
  ...
  const generateHighQualityFeedback = (reasoning: string, isCorrect: boolean, termScore: number, usedCategories: number, hasExplanation: boolean, hasComparison: boolean) => {
    let feedback = '';
  ...
  const generateBasicFallbackFeedback = (reasoning: string, isCorrect: boolean) => {
    const reasoningLength = reasoning.trim().length;
    
    if (reasoningLength < 20) {
      return {
        feedback: "Deine Begründung ist zu kurz. Erkläre deine Gedanken ausführlicher.",
        qualityScore: 2,
        suggestions: ["Schreibe mehr", "Verwende Fachbegriffe", "Erkläre genauer"]
      };
    }
    
    return {
      feedback: isCorrect 
        ? "Gute Antwort! Deine Begründung zeigt Verständnis."
        : "Deine Begründung ist nachvollziehbar, aber die Antwort ist nicht korrekt.",
      qualityScore: isCorrect ? 6 : 4,
      suggestions: [
        isCorrect ? "Weiter so!" : "Überprüfe die Grundlagen",
        "Verwende mehr Fachbegriffe",
        "Erkläre detaillierter"
      ]
    };
  };
  ...
  const handleAnswerSubmit = async () => {
    ...
    try {
      ...
      // Store the attempt with reasoning for teacher dashboard
      const attemptData = {
        id: `attempt-${Date.now()}`,
        user_id: user?.id,
        ...
        selected_answer: selectedOption?.option_text || answer,
        reasoning: userReasoning,
        points_got: pointsAwarded,
        dose_got_msv: doseReceived,
        ai_feedback: aiEvaluation.feedback,
        ai_quality_score: aiEvaluation.qualityScore,
        ai_suggestions: aiEvaluation.suggestions,
        is_correct: isCorrect,
        timestamp: new Date().toISOString()
      };

      // Store in localStorage for demo purposes (teacher dashboard will read this)
      const existingAttempts = JSON.parse(localStorage.getItem('demo_attempts') || '[]');
      existingAttempts.push(attemptData);
      localStorage.setItem('demo_attempts', JSON.stringify(existingAttempts));
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-blue-700 mb-3">
                  ...
                  Unser fortschrittliches KI-System analysiert deine Begründung und gibt dir sofortiges, personalisiertes Feedback!
                </p>
                <div className="bg-white/50 rounded-lg p-3">
                  <p className="text-sm text-blue-600">
                    <strong>Das System bewertet:</strong> Fachliche Korrektheit • Verwendung von Fachbegriffen • 
                ...
                <div className="mt-3 p-2 bg-green-100 rounded text-sm text-green-700">
                  💡 <strong>Tipp:</strong> Verwende Fachbegriffe wie "Radioaktivität", "Strahlung", "Dosis" und erkläre deine Überlegungen ausführlich!
                </div>
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div className="mb-8">
              <div className="flex items-center space-x-2 mb-4">
                ...
                <h3 className="text-lg font-medium text-gray-800">Wissenschaftliche Begründung (Pflichtfeld):</h3>
              </div>
              
              <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-4 mb-4">
                <p className="text-sm text-purple-700">
                  <strong>🤖 KI-Analyse:</strong> Erkläre deine Antwort wissenschaftlich fundiert. 
                  Das System bewertet deine Begründung auf fachliche Korrektheit, Verwendung von Fachbegriffen 
                  und logische Struktur. Je besser deine Begründung, desto hilfreicher das Feedback!
                </p>
                <div className="mt-2 text-xs text-purple-600">
                  💡 <strong>Verwende Fachbegriffe wie:</strong> Radioaktivität, Strahlung, Dosis, Zerfall, Isotop, Schutz, Abschirmung
                </div>
              </div>
          ...
          >
            <div className="space-y-8">
                ...
                <div className="flex items-start space-x-3 mb-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      <h4 className="font-semibold text-purple-800">🤖 KI-Bewertung deiner Begründung:</h4>
                      {feedback.qualityScore && (
                        <div className={`px-3 py-1 rounded-full text-sm font-medium border ${getQualityColor(feedback.qualityScore)}`}>
              ...
              {feedback.doseReceived > 0 && (
                <div className="flex items-center justify-center space-x-2 text-yellow-700 bg-yellow-50 p-3 rounded-lg border border-yellow-200">
